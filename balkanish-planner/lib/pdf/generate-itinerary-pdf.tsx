 ...
 */
export async function generateItineraryPdfBlob(itinerary: GeneratedItinerary, input: PlannerInput): Promise<Blob> {
  const [{ pdf }, { ItineraryPdfDocument }] = await Promise.all([
    import("@react-pdf/renderer"),
    import("@/components/planner/itinerary-pdf"),
  ]);
  return pdf(<ItineraryPdfDocument itinerary={itinerary} input={input} />).toBlob();
}
